...
// tierB: clk-divider table walk

#[no_mangle]
pub extern "C" fn cgir_get_table_div(table: *const ClkDivTable, val: u32) -> u32 {
    unsafe {
        let mut clkt = table;
        while (*clkt).div != 0 {
            if (*clkt).val == val {
...
#[no_mangle]
pub extern "C" fn cgir_get_table_val(table: *const ClkDivTable, div: u32) -> u32 {
    unsafe {
        let mut clkt = table;
        while (*clkt).div != 0 {
            if (*clkt).div == div {
